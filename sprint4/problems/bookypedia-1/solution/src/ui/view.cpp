...
bool View::AddAuthor(std::istream& cmd_input) const {
    try {
        ...
        if (!std::getline(cmd_input, name)) {
            throw std::invalid_argument("Author name is empty"s);
        }
...
std::vector<detail::AuthorInfo> View::GetAuthors() const {
    ...
    for (const auto& author: use_cases_.GetAuthors()) {
        dst_autors.emplace_back(detail::AuthorInfo{author.GetId().ToString(), author.GetName()});
    }
    return dst_autors;
}
...
std::vector<detail::BookInfo> View::GetAuthorBooks(const std::string& author_id) const {
    std::vector<detail::BookInfo> books;
    //TODO: ИЗБАВИТЬСЯ ОТ ЗАВИСИМОСТИ util::TaggedUUID<domain::detail::AuthorTag>
    for (const auto& book: use_cases_.GetAuthorBooks(domain::Author(util::TaggedUUID<domain::detail::AuthorTag>::FromString(author_id), ""s))) {
        books.emplace_back(detail::BookInfo{book.GetTitle(), book.GetPublicationYear()});
    }
    return books;
}
